...
// Реализация LostObject
LostObject::LostObject(int id, int type, double x, double y)
    : id_(id), type_(type), x_(x), y_(y) {}

int LostObject::GetId() const { 
    return id_; 
}

int LostObject::GetType() const { 
    return type_; 
}

double LostObject::GetX() const { 
    return x_; 
}

double LostObject::GetY() const { 
    return y_; 
}

// Реализация GameSession
GameSession::GameSession(const Map& map) 
    : map_(map), next_loot_id_(0) {}
...
size_t GameSession::GetLootersCount() const { 
    return 1; // Упрощённо: всегда 1 мародёр
}
